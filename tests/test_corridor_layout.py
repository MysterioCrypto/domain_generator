...
def corridor_feature(feature_id: str, *, control_point_count: int = 3, curvature: float = 0.5) -> ResolvedFeature:
...
def make_plan(*, features: tuple[ResolvedFeature, ...], constraints: tuple[CompiledConstraint, ...] = (), seed: int = 123456) -> GenerationPlan:
    return GenerationPlan(
        plan_version="0.1",
        source=PlanSource(spec_id="corridor-test", spec_schema_version="0.1", spec_fingerprint="sha256:test", generator_version="0.1.0.dev0"),
        seed=seed,
...
def test_triangular_sampler_rng_v1_golden_vector() -> None:
    recipe = RangeParameter(type=ParameterType.FLOAT, min=0.1, max=0.9, sampler=TriangularSampler(mode=0.35))
    factory = RngFactory(123456)
    stream = factory.stream(RngKey(attempt_index=2, stage=RngStage.LAYOUT, scope=("feature", "corridor-01", "parameter", "curvature"), purpose="sample"))
...
def test_point_to_corridor_distance_constraint_is_evaluated() -> None:
    constraint = CompiledConstraint(id="point-near-corridor", strength=ConstraintStrength.HARD, evaluator=CompiledEvaluator(type="distance", subject=CompiledFeatureRef(feature_id="point-01", part=FeaturePart.WHOLE), target=CompiledFeatureRef(feature_id="corridor-01", part=FeaturePart.WHOLE)), predicate=CompiledPredicate(type="less_or_equal", value=1000.0), unit="km")
    plan = make_plan(features=(point_feature("point-01"), corridor_feature("corridor-01")), constraints=(constraint,))
...
def test_corridor_start_part_can_be_used_as_point_distance_subject() -> None:
    constraint = CompiledConstraint(id="start-near-origin", strength=ConstraintStrength.HARD, evaluator=CompiledEvaluator(type="distance", subject=CompiledFeatureRef(feature_id="corridor-01", part=FeaturePart.START), target=CompiledPoint(x_km=0.0, y_km=0.0)), predicate=CompiledPredicate(type="less_or_equal", value=1000.0), unit="km")
...
def test_area_without_required_parameters_fails_as_capability_error() -> None:
    feature = ResolvedFeature(id="area-01", metadata=FeatureMetadata(source_preset="test-area"), family=FeatureFamily.TERRAIN, layout=GeometryLayoutRecipe(shape=GeometryShape.AREA, parameters={}), effect=EffectRecipe(stage="terrain", operator="test-effect"))

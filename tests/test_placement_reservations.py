...
from domain_generator.layout.reservations import (
    ...
    *,
    ...
    return CompiledConstraint(
        ...
        evaluator=CompiledEvaluator(
            type=evaluator_type,
            subject=CompiledFeatureRef(
                feature_id=subject_feature_id,
                part=FeaturePart.WHOLE,
            ),
            target=target,
        ),
...
def test_inside_rectangle_intersects_allowed_region() -> None:
    constraint = hard_constraint(
        "inside-west",
        evaluator_type="contained_fraction",
        ...
        predicate_type="greater_or_equal",
        value=1.0,
    )
...
def test_outside_rectangle_creates_hole() -> None:
    constraint = hard_constraint(
        "outside-center",
        evaluator_type="overlap_fraction",
        target=CompiledRectangle(
            min_x_km=40.0,
            max_x_km=60.0,
            min_y_km=30.0,
...
def test_near_literal_point_builds_buffer_region() -> None:
    constraint = hard_constraint(
        "near-point",
        evaluator_type="distance",
        target=CompiledPoint(x_km=50.0, y_km=40.0),
        predicate_type="less_or_equal",
        value=5.0,
        unit="km",
    )
...
def test_constraint_order_is_canonicalized_by_constraint_id() -> None:
    ...
    plan_za = make_plan(
        features=(reservation_feature(),),
        constraints=(constraint_z, constraint_a),
    )
    plan_az = make_plan(
        features=(reservation_feature(),),
        constraints=(constraint_a, constraint_z),
    )
...
def test_empty_reservation_is_structural_but_rejects_layout_validation() -> None:
    ...
    reservation_invariant = next(
        result
        for result in validation.engine_invariants.results
        if result.id == "layout-placement-reservations-nonempty"
    )
...
def test_reservation_and_concrete_geometry_coexist_in_one_candidate() -> None:
    constraint = hard_constraint(
        "near-concrete-point",
        evaluator_type="distance",
        target=CompiledFeatureRef(feature_id="point-01", part=FeaturePart.WHOLE),
        predicate_type="less_or_equal",
        value=8.0,
        unit="km",
    )
    plan = make_plan(
        features=(reservation_feature(), point_feature()),
...
def test_band_whole_target_is_explicit_capability_error() -> None:
    constraint = hard_constraint(
        "near-band",
        evaluator_type="distance",
        target=CompiledFeatureRef(feature_id="band-01", part=FeaturePart.WHOLE),
        predicate_type="less_or_equal",
        value=5.0,
        unit="km",
    )
    plan = make_plan(
        features=(reservation_feature(), band_feature()),
        constraints=(constraint,),
    )
...
def test_deferred_feature_as_constraint_target_is_explicit_capability_error() -> None:
    constraint = CompiledConstraint(
        ...
        evaluator=CompiledEvaluator(
            type="distance",
            subject=CompiledPoint(x_km=10.0, y_km=10.0),
            target=CompiledFeatureRef(feature_id="poi-01", part=FeaturePart.WHOLE),
        ),
        predicate=CompiledPredicate(type="less_or_equal", value=5.0),
        unit="km",
    )
    plan = make_plan(features=(reservation_feature(),), constraints=(constraint,))

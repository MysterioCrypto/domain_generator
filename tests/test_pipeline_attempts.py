...
def test_attempt_stops_at_first_failed_stage_without_retry() -> None:
    ...
    steps = make_steps(
        calls=calls,
        fail_stage_by_attempt={0: ValidationStage.HYDROLOGY},
    )
    ...
    assert calls == [
        (0, ValidationStage.LAYOUT),
        (0, ValidationStage.TERRAIN),
        (0, ValidationStage.HYDROLOGY),
    ]
...
def test_target_one_is_first_valid_behavior() -> None:
    ...
    steps = make_steps(
        calls=calls,
        fail_stage_by_attempt={0: ValidationStage.LAYOUT},
        ranking_by_attempt={1: (0.7, 0.2)},
    )

    result = run_generation(
        plan=minimal_plan(),
        config=config(max_attempts=5, target_valid_candidates=1),
        steps=steps,
    )
...
def test_multiple_valid_candidates_use_minimax_then_mean_then_attempt_index() -> None:
    ...
    result = run_generation(
        plan=minimal_plan(),
        config=config(max_attempts=10, target_valid_candidates=3),
        steps=steps,
    )
...
def test_exact_ranking_tie_prefers_lower_attempt_index() -> None:
    ...
    steps = make_steps(
        calls=calls,
        ranking_by_attempt={0: (0.1, 0.8), 1: (0.1, 0.8)},
    )

    result = run_generation(
        plan=minimal_plan(),
        config=config(max_attempts=2, target_valid_candidates=2),
        steps=steps,
    )
...
def test_budget_exhaustion_with_some_valid_candidates_selects_best_available() -> None:
    ...
    steps = make_steps(
        calls=calls,
        fail_stage_by_attempt={1: ValidationStage.SURFACE},
        ranking_by_attempt={0: (0.3, 0.9), 2: (0.2, 0.6)},
    )

    result = run_generation(
        plan=minimal_plan(),
        config=config(max_attempts=3, target_valid_candidates=3),
        steps=steps,
    )
...
def test_no_valid_candidate_raises_generation_failure() -> None:
    ...
    steps = make_steps(
        calls=calls,
        fail_stage_by_attempt={
            0: ValidationStage.LAYOUT,
            1: ValidationStage.TERRAIN,
            2: ValidationStage.PLACEMENT,
        },
    )
    ...
    with pytest.raises(GenerationFailure) as exc_info:
        run_generation(
            plan=minimal_plan(),
            config=config(max_attempts=3, target_valid_candidates=2),
            steps=steps,
        )
...
def test_observability_flags_do_not_change_semantic_result() -> None:
    ...
    result_a = run_generation(
        plan=plan,
        config=config(max_attempts=2, target_valid_candidates=2, debug=False),
        steps=make_steps(calls=calls_a, ranking_by_attempt=rankings),
    )
    result_b = run_generation(
        plan=plan,
        config=config(max_attempts=2, target_valid_candidates=2, debug=True),
        steps=make_steps(calls=calls_b, ranking_by_attempt=rankings),
    )
...
def test_completed_attempt_returns_domain_candidate_with_all_stage_validations() -> None:
    ...
    outcome = run_attempt(
        plan=minimal_plan(),
        attempt_index=4,
        steps=make_steps(calls=calls, ranking_by_attempt={4: (0.0, 1.0)}),
    )

...
def test_generated_points_are_inside_domain() -> None:
    ...
    candidate = generate_point_layout(generation_plan, attempt_index=11, rng_factory=RngFactory(generation_plan.seed))
...
def test_point_inside_rectangle_constraint_can_reject_without_regeneration() -> None:
    constraint = CompiledConstraint(
        ...
        evaluator=CompiledEvaluator(
            ...
            target=CompiledRectangle(min_x_km=120.0, max_x_km=120.0, min_y_km=100.0, max_y_km=100.0),
        ),
    ...
    candidate = generate_point_layout(generation_plan, attempt_index=0, rng_factory=RngFactory(generation_plan.seed))
...
def test_point_distance_constraint_is_measured_in_km() -> None:
    constraint = CompiledConstraint(
        id="alpha-near-beta",
        strength=ConstraintStrength.HARD,
        evaluator=CompiledEvaluator(type="distance", subject=CompiledFeatureRef(feature_id="alpha"), target=CompiledFeatureRef(feature_id="beta")),
...
def test_stage_handler_sets_candidate_state_layout() -> None:
    generation_plan = plan(point_feature("alpha"))
    context = AttemptContext(plan=generation_plan, attempt_index=4, rng_factory=RngFactory(generation_plan.seed))
...
def test_non_point_geometry_is_explicitly_unsupported() -> None:
    feature = point_feature("alpha").model_copy(update={"layout": GeometryLayoutRecipe(mode="geometry", shape=GeometryShape.AREA, parameters={})})

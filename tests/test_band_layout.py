...
from domain_generator.layout.geometry import LayoutCapabilityError, generate_geometry_layout, validate_geometry_layout
from domain_generator.pipeline.rng import RngFactory


def band_feature(feature_id: str, *, control_point_count: int = 3, curvature: float = 0.5, width_sample_count: int = 5, width_recipe=None) -> ResolvedFeature:
    ...
    return ResolvedFeature(
        ...
        layout=GeometryLayoutRecipe(
            ...
            parameters={
                "control_point_count": FixedParameter(type=ParameterType.INTEGER, value=control_point_count),
                "curvature": FixedParameter(type=ParameterType.FLOAT, value=curvature),
                "width_km": width_recipe,
                "width_sample_count": FixedParameter(type=ParameterType.INTEGER, value=width_sample_count),
            },
        ),
...
def corridor_feature(feature_id: str) -> ResolvedFeature:
    return ResolvedFeature(
        ...
        layout=GeometryLayoutRecipe(
            ...
            parameters={
                "control_point_count": FixedParameter(type=ParameterType.INTEGER, value=3),
                "curvature": FixedParameter(type=ParameterType.FLOAT, value=0.5),
            },
        ),
        effect=EffectRecipe(stage="terrain", operator="test-effect"),
    )
...
def make_plan(*, features: tuple[ResolvedFeature, ...], constraints: tuple[CompiledConstraint, ...] = (), seed: int = 123456) -> GenerationPlan:
    return GenerationPlan(
        plan_version="0.1",
        source=PlanSource(spec_id="band-test", spec_schema_version="0.1", spec_fingerprint="sha256:test", generator_version="0.1.0.dev0"),
        seed=seed,
...
def test_band_width_profile_has_deterministic_even_t_samples() -> None:
    ...
    assert tuple(sample.t for sample in geometry.width_profile) == (0.0, 0.25, 0.5, 0.75, 1.0)


def test_fixed_band_width_is_constant_across_profile() -> None:
    plan = make_plan(features=(band_feature("band-01", width_sample_count=7, width_recipe=FixedParameter(type=ParameterType.FLOAT, value=18.0)),))
    candidate = generate_geometry_layout(plan, attempt_index=2, rng_factory=RngFactory(plan.seed))
    geometry = candidate.geometry_realizations["band-01"]
    assert isinstance(geometry, BandGeometry)
...
def test_width_sample_count_does_not_change_endpoint_widths() -> None:
    width_recipe = RangeParameter(type=ParameterType.FLOAT, min=5.0, max=20.0, sampler=UniformSampler())
    short_plan = make_plan(features=(band_feature("band-01", width_sample_count=3, width_recipe=width_recipe),))
    long_plan = make_plan(features=(band_feature("band-01", width_sample_count=9, width_recipe=width_recipe),))
...
def test_band_and_corridor_use_distinct_geometry_namespaces() -> None:
    ...
    band_candidate = generate_geometry_layout(band_plan, attempt_index=5, rng_factory=RngFactory(band_plan.seed))
    corridor_candidate = generate_geometry_layout(corridor_plan, attempt_index=5, rng_factory=RngFactory(corridor_plan.seed))
...
def test_band_centerline_vertices_stay_inside_domain() -> None:
    plan = make_plan(features=(band_feature("band-01", control_point_count=12, curvature=1.0),))
...
def test_band_start_part_can_be_used_as_point_distance_subject() -> None:
    constraint = CompiledConstraint(id="band-start-near-origin", strength=ConstraintStrength.HARD, evaluator=CompiledEvaluator(type="distance", subject=CompiledFeatureRef(feature_id="band-01", part=FeaturePart.START), target=CompiledPoint(x_km=0.0, y_km=0.0)), predicate=CompiledPredicate(type="less_or_equal", value=1000.0), unit="km")
...
def test_band_center_part_is_resolved_at_half_arc_length() -> None:
    constraint = CompiledConstraint(id="band-center-near-origin", strength=ConstraintStrength.HARD, evaluator=CompiledEvaluator(type="distance", subject=CompiledFeatureRef(feature_id="band-01", part=FeaturePart.CENTER), target=CompiledPoint(x_km=0.0, y_km=0.0)), predicate=CompiledPredicate(type="less_or_equal", value=1000.0), unit="km")
    plan = make_plan(features=(band_feature("band-01"),), constraints=(constraint,))
    candidate = generate_geometry_layout(plan, attempt_index=7, rng_factory=RngFactory(plan.seed))
...
def test_band_width_profile_invariants_pass_for_generated_geometry() -> None:
    width_recipe = RangeParameter(type=ParameterType.FLOAT, min=8.0, max=24.0, sampler=UniformSampler())
    plan = make_plan(features=(band_feature("band-01", width_sample_count=6, width_recipe=width_recipe),))
    candidate = generate_geometry_layout(plan, attempt_index=9, rng_factory=RngFactory(plan.seed))
